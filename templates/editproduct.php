...
if(isset($_SERVER['HTTP_REFERER'])){
    ...
    if($from_products || $from_self){
        if($validCodes){
            ...
            if(isset($_GET['id'])){
                ...
                if(preg_match("/[0-9]{1,5}/", $id)) $product = json_decode($smt_smart_commerce_getProduct($id), true);
            }

            $productImages = json_encode(array_map(function($item){return array(

                'src' => sanitize_url($item["src"]), 
    ...
    if(isset($_POST['save'])){
        ...
        if(isset($_POST['product-name'])){
            $productName = sanitize_text_field($_POST['product-name']);
        ...
        if(isset($_POST['product-regular-price']) && $_POST['product-regular-price'] != $product['regular_price']){
            $regularPrice = sanitize_text_field($_POST['product-regular-price']);
            if(preg_match($priceRegex, $regularPrice)) $data['regular_price'] = $regularPrice;
        } 

        if(isset($_POST['product-sale-price']) && $_POST['product-sale-price'] != $product['sale_price']){
            $saleprice = sanitize_text_field($_POST['product-sale-price']);
            if(preg_match($priceRegex, $saleprice)) $data['sale_price'] = $saleprice;
        } 

        if(isset($_POST['tax-class']) && $_POST['tax-class'] != $product['tax_class']){
        ...
        if(isset($_POST['wp-commerce-product-short-description']) && $_POST['wp-commerce-product-short-description'] != $product['short_description']){
       ...
       if($validCodes) {
           $saveProduct = json_decode($smt_smart_commerce_updateProduct($id, $data), true);

           if($saveProduct['error']) $error = $saveProduct['message'];
        };


        if(!$error) header("Location: " . sanitize_url($link . "/" . $products_page . "?id=1"));
    } 

    // Use this color in hidden input to change brand color dynamically with jQuery
...
<body>
        ...
        <form enctype="multipart/form-data" action="" method="post" id="addeditproduct-form">
            ...
            <div id="title-price" class="flex-fields">
                ...
                <div>
                    ...
                    <input type="text" name="product-name" id="name" value="<?php echo esc_attr($product['name']) ?>">
                </div>
            </div>
            ...
            <div id="general">
                ...
                <div class="flex-container">
                    <div class="flex-container-vertical">
                        ...
                        <input type="text" name="product-regular-price" id="regular-price" value="<?php echo esc_attr($product['regular_price'])?>">
                    </div>

                    <div class="flex-container-vertical">
                        <label for="product-sale-price" class="">Sale Price</label>
                        <input type="text" name="product-sale-price" id="sale-price" value="<?php echo esc_attr($product['sale_price']) ?>">
                    </div>
            ...
            <div id="inventory">
                ...
                <div class="inventory-container">
                    <div id="sku" class="flex-container">
                        ...
                        <input type="text" name="product-sku" id="sku-input" value="<?php echo esc_attr($product['sku']) ?>">
                    </div>
                </div>
            </div>
    ...
    <?php endif;?>
    <div id="errors"></div>
    <input type="hidden" id="brand-color" value="<? echo esc_html($color)?>">
    <?php wp_footer(); ?>
</body>
</html>